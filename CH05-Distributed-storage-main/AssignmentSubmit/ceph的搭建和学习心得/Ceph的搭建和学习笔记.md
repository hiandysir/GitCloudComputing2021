...
本文最后也是将ceph搭建成功，搭建过程相对来说比较顺利。

**这里补充一些搭建的小提示：**

- 首先要保证三台虚拟机能够联通，且能够连接网络。
- 其次就是熟悉docker的使用，比如说，启动mon节点时间，可能会将主机ip漏写，写错等等。只能强制将docker容器删除。
- 最后的话，本文是直接进入root用户进行部署，相对与普通用户而言省去了sudo之类反复输入密码的繁琐过程。
